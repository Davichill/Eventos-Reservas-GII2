...
// backend/web/js/admin/graficos.js

function inicializarGraficos(datos) {
    console.log("Renderizando con datos:", datos);
    
    // Gráfico de Cobros (Pie)
    const ctxPie = document.getElementById('pagoPieChart');
    if (ctxPie && datos.length > 0) {
        new Chart(ctxPie, {
            type: 'pie',
            data: {
        ...
        // Actualizar el número del total proyectado
        let suma = datos.reduce((a, b) => a + parseFloat(b.total), 0);
        document.getElementById('totalInfo').innerText = '$' + suma.toLocaleString();
    }
}

// Esta es la función que está fallando según tu captura (Error 404)
// Por ahora, para que no de error, puedes dejarla vacía o corregir la URL
function cargarDatosCalendario() {
    // Si vas a usar AJAX en Yii2, la URL debe ser:
    // url: 'index.php?r=dashboard/get-calendario-data'
    console.log("Pausa de carga de calendario para evitar 404");
}